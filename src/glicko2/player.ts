...
export class Player {
  ...
  public addResult = (opponent: Player, outcome: Outcome): Player => {
    ...
    const oppRating = (opponent.rating - opponent.defaultRating) / Player.ScalingFactor
    const oppRD = opponent.rd / Player.ScalingFactor
  ...
  private _variance = (): number => {
    let sum = 0
    for (let i = 0, len = this.oppRatings.length; i < len; i++) {
      const E = this._E(this.oppRatings[i], this.oppRDs[i])
  ...
  private _fFactory = (delta: number, v: number, a: number) => {
    ...
    return (x: number) => {
      return (
        (Math.exp(x) * (Math.pow(delta, 2) - Math.pow(player._rd, 2) - v - Math.exp(x))) /
          (2 * Math.pow(Math.pow(player._rd, 2) + v + Math.exp(x), 2)) -
        (x - a) / Math.pow(player._tau, 2)
      )
    }
  }
}
